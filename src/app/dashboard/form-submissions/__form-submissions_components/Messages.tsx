...
import { Card, CardHeader, CardTitle, CardDescription, CardContent } from "@chroniconl/ui/card"
import { Input } from "@chroniconl/ui/input"
import { Table, TableHeader, TableRow, TableHead, TableBody, TableCell } from "@chroniconl/ui/table"
...
export default function Messages({ messages }: MessagesProps) {
	...
	const filteredMessages = messageList.filter((message) =>
		Object.values(message).some((value) => value.toString().toLowerCase().includes(searchTerm.toLowerCase())),
	)
		...
		<div>
			<Card className="w-full">
				<CardHeader>
					...
					<CardDescription>View and manage all contact us messages.</CardDescription>
				</CardHeader>
				<CardContent>
						...
						<TableBody>
							{sortedMessages.map((message) => (
								<TableRow key={message.id}>
									...
									<TableCell>
										<Badge variant={message.status === "seen" ? "secondary" : "outline"}>{message.status}</Badge>
									</TableCell>
									...
									<TableCell>
										...
										<Button variant="ghost" size="icon" onClick={() => handleShowModal(message)}>
