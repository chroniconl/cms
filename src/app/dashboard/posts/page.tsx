...
import {
  sortParamsToSupabaseQuery,
  validateSort,
} from './_posts_utils/validateSort'
...
export default async function PostsPage({
  ...
  const searchParamsRecords =
    parseInt(searchParams.records as string) || DEFAULT_RECORDS
  const records =
    searchParamsRecords > 0 ? searchParamsRecords : DEFAULT_RECORDS
  const sort = sortParamsToSupabaseQuery(
    validateSort(searchParams.sort as string),
  )
