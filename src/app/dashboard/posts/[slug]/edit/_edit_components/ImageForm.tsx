...
export default function ImageForm({
	...
	const state__imageUrl = useMetaFormStore((state) => state.imageUrl);
	const state__setImageUrl = useMetaFormStore((state) => state.setImageUrl);
	const state__imageId = useMetaFormStore((state) => state.imageId);
	const state__setImageId = useMetaFormStore((state) => state.setImageId);

	useEffect(() => {
		if (props__imageUrl) {
			state__setImageUrl(props__imageUrl);
		}
		if (props__imageId) {
			state__setImageId(props__imageId);
		}
	}, []);

	const handleDeleteImage = async () => {
		if (!state__imageId) {
			console.error('No image ID to delete. If this was unexpected, please report it to the developer');
			return;
		}
	...
	const onSubmit = async (data: any) => {
		...
		if (error) {
			toast({
				title: 'Error',
				description: "Something went wrong updating the images metadata. Please try again later.",
				variant: 'destructive',
			})
			return;
		}
						...
						<AlertDialog>
							...
							<AlertDialogContent>
								<AlertDialogHeader>
									...
									<AlertDialogDescription>
										This action cannot be undone. You will not be able to recover this image from the database.
									</AlertDialogDescription>
								</AlertDialogHeader>
								<AlertDialogFooter>
									<AlertDialogCancel>Cancel</AlertDialogCancel>
									<AlertDialogAction onClick={handleDeleteImage}>Yes, I'm sure</AlertDialogAction>
								</AlertDialogFooter>
							</AlertDialogContent>
						</AlertDialog>
					)}
						...
						>
							<div>
								...
								<Controller
									...
									control={control}
									defaultValue=""
									render={({ field }) => <Input type="text" placeholder="Alt text" {...field} />}
								/>
							</div>
							<div>
								...
								<Controller
									...
									control={control}
									defaultValue=""
									render={({ field }) => <Input type="text" placeholder="Photo by ..." {...field} />}
								/>
							</div>
							<Button className="mt-4" type="submit">Update Image Meta</Button>
						</form>
					</div>
				) : (
					<UploadDropzone
