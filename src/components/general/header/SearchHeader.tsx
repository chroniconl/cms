...
export function SearchHeader() {
	...
	const debounceSearch = useCallback(debounce(sendSearchRequest, 500), [sendSearchRequest]);
				...
				<DialogContent className="sm:max-w-md">
					...
					<ScrollArea className="h-72 rounded-md border">
						...
						<div className="flex flex-col gap-2 p-2">
							{
								Object.values(results.searchResults).length > 0 && Object.values(results.searchResults).map((result) => (
									...
									>
										...
										<div>
											<span className="block text-sm font-bold text-secondary-foreground text-nowrap overflow-ellipsis">{result.title}</span>
											<span className="block text-sm text-secondary-foreground">{formatDate(result.created_at)}</span>
										</div>
									</Link>
								))
							}
