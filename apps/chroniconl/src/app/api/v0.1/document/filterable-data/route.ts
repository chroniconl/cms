...
export async function POST(request: Request) {
  ...
  const { data: tagsThatExistData, error: tagsThatExistError } =
    await findTagsThatAlreadyExistInDatabase(tags)
  ...
  const { data: newTagsData, error: newTagsError } = await createTagsInDatabase(
    tagsToInsert,
    userData,
  )
  if (newTagsError) {
  ...
  const tagsToRetrieveFromDatabase = tags.filter((tag) =>
    tagsThatExistData?.find((tagData) => tagData.name === tag),
  )

  // retrieve tags that are already in the database
  const { data: tagsRetrievedData, error: tagsRetrievedError } =
    await getTagsThatAlreadyExistInDatabase(tagsToRetrieveFromDatabase)
  ...
  const {
    data: existingPostTagRelationshipData,
    error: existingPostTagRelationshipError,
  } = await supabase
  ...
  const postTagRelationshipDataIds = existingPostTagRelationshipData?.map(
    ({ postTagRelationship }) => postTagRelationship.tag_id,
  )

  // remove the tags that already exist in the post_tag_relationship table
  const tagsDataMergedWithoutExistingRelationships = tagsDataMerged.filter(
    (tag) => !postTagRelationshipDataIds.includes(tag.id),
  )

  // create post_tag_relationship in database
  const { error: postTagRelationshipError } =
    await createTagsRelationshipsInDatabase(
      tagsDataMergedWithoutExistingRelationships,
      {
